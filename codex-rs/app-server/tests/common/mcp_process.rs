...
impl McpProcess {
    ...
    /// Send a `thread/realtime/call/create` JSON-RPC request (v2).
    pub async fn send_thread_realtime_call_create_request(
        &mut self,
        params: ThreadRealtimeCallCreateParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
        self.send_request("thread/realtime/call/create", params)
            .await
    }
