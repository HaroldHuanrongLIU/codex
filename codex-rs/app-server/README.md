...
- `realtime/call/create` — create a WebRTC realtime call from a local SDP offer (experimental); returns `{ sdp }` with the remote SDP. API key auth posts the SDP directly to `/v1/realtime/calls`; ChatGPT auth uses the codex-backend proxy and requires `session`.
